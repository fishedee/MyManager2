...
func (this *UserAoTest) testAdd(userFish *User, userEdward *User) {
	...
	*userFish = User{
	...
	_, err := this.UserAo.Add_WithError(userFish2)
...
func (this *UserAoTest) testAdd() (User, User) {
	//添加用户
	userFish := User{
		UserId:   10001,
		Name:     "fish",
...
func (this *UserAoTest) testmod(userEdward User) User {
	//修改类型
	this.UserAo.ModType(userEdward.UserId, 1)
	userEdward.Type = 1
	getEdwardData2 := this.UserAo.Get(userEdward.UserId)
...
func (this *UserAoTest) TestBasic() {
	...
	userFish, userEdward := this.testAdd()
	this.testSearch(userFish, userEdward)
	userEdward = this.testmod(userEdward)
	this.testPassword(userEdward)
	this.testDel(userEdward)
